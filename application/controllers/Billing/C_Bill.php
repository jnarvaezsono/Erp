...
class C_Bill extends Controller {
    ...
    function ListBill($factura, $cliente, $f_ini, $f_fin) {
        ...
        foreach ($rows['result'] as $v) {
            ...
            $btn    .= ((isset($BtnSend)) && ($v->est_id == 4 || $v->est_id == 44))?'<li onclick="PrintXmlProductivo(' . $v->factura_id . ',0)"><a href="#"></i><i class="fa fa-fw fa-send"></i> Enviar CEN</a></li>':'';
            $btn    .= ((isset($BtnDownXml)) && ($v->est_id == 4 || $v->est_id == 44))?'<li onclick="OpenXmlProductivo(' . $v->factura_id . ',0)"><a href="#"></i><i class="fa fa-fw fa-download"></i> Descargar XML</a></li>':'';
            
            if($this->session->IdRol == 1){
                $btn    .= ((isset($BtnSend)) && ($v->est_id == 4 || $v->est_id == 44))?'<li class="bg-danger" onclick="PrintXmlProductivo(' . $v->factura_id . ',1)"><a href="#"></i><i class="fa fa-fw fa-send"></i> Enviar PILOTO</a></li>':'';
                $btn    .= ((isset($BtnDownXml)) && ($v->est_id == 4 || $v->est_id == 44))?'<li  class="bg-danger"  onclick="OpenXmlProductivo(' . $v->factura_id . ',1)"><a href="#"></i><i class="fa fa-fw fa-download"></i> Descargar XML PILOTO</a></li>':'';
            }
...
//        $html = $this->load->view('Billing/Bill/V_Pdf',array('v'=>$result));
    ...
    function GenerateXml($bill,$piloto, $send = false) {
...
//                }
        ...
        if ($send) {
            ...
            $host = ($piloto == 0)?'sftpebz.cen.biz':'sftp-piloto.cen.biz';
            $port = 220;
