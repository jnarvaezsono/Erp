...
<html lang="en">
    ...
    <body>
        ...
        <header class="clearfix">
            ...
            <?php if($printOrder == 0): ?>
            <h1><b>PRESUPUESTO DE PRENSA No. <?=$v->id?> <?=(count($bill)>0)?' FACTURA '.$bill->factura_id:''?></b></h1>
            <?php else: ?>
            <h1><b>ORDEN DE PRENSA No. <?=$orden->ord_id?></b></h1>
            <?php endif; ?>
            <table id="head" >
                <tr>
                ...
                <tr><td class="th-cab">SERVICIO</td><td><?=strtoupper($v->servicio)?></td><td class="th-cab">MEDIO</td><td class="td-cab"><?=$v->medio?></td><td class="th-cab">COTIZACIÓN</td><td><?=$v->cotizacion?></td></tr>
        ...
        <main>
            <table id="aviso" >
                <tbody>
                    ...
                    <tr>
                        <th style="border-top: 1px solid #C1CED9;">TAMAÑO</th>
                        <th style="border-top: 1px solid #C1CED9;">TINTAS</th>
                        <th style="border-top: 1px solid #C1CED9;">No.AVISOS</th>
                        <th style="border-top: 1px solid #C1CED9;">TARIFA</th>
                        <th style="border-top: 1px solid #C1CED9;">VLR.UNITARIO</th>
                        <th style="border-top: 1px solid #C1CED9;">VLR.TOTAL</th>
                    </tr>
                        <tr>
                            <td class="desc">
                                <?php if ($row->detavi_numcolum == 0.000 || $row->detavi_numcentim == 0.000): ?>
                                    <?=$row->detavi_tamano?>
                                <?php else: ?>
                                    <?=number_format($row->detavi_numcolum) . ' Columna(s) * ' . number_format($row->detavi_numcentim) . ' Cms';?>
                                <?php endif; ?>
                            </td>
                            <td class="qty"><?=$row->tinta ?></td>
                            <td class="qty"><?=$row->detavi_numavisos; ?></td>
                            <td class="qty">$ <?=number_format($row->detavi_tarifa,0,"","."); ?></td>
                            <td class="qty">$ <?=number_format($row->detavi_valor,0,"","."); ?></td>
                            <td class="total">$ <?=number_format($row->detavi_total,0,"","."); ?></td> 
                        </tr>
                        <tr>
                            <td style="text-align:left">TITULO :</td><td style="text-align:left" colspan="5"><?=$row->detavi_titulo?></td>
                        </tr>
                        <tr>
                            <td style="text-align:left">PÁGINA :</td><td style="text-align:left" colspan="5"><?=$row->pagina?></td>
                        </tr>
                        <tr>
                            <td style="text-align:left">POSICIÓN :</td><td style="text-align:left" colspan="5"><?=$row->detavi_posicion?></td>
                        </tr>
                        <tr>
                            <td style="text-align:left">FECHA INSERCIÓN :</td><td style="text-align:left" colspan="5"><?=wordwrap($row->detavi_fechinser, 132, "<br />",1)?></td>
                        </tr>
                        <?php if(!empty($row->detavi_detalle)): ?>
                            <tr>
                                <td style="text-align:left">DETALLE :</td><td style="text-align:left" colspan="5"><?=$row->detavi_detalle?></td>
                            </tr>
                        <?php endif; ?>
                    <?php 
