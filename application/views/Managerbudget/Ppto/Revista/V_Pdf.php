...
<html lang="en">
    ...
    <body>
        ...
        <header class="clearfix">
            ...
            <?php if($printOrder == 0): ?>
            <h1><b>PRESUPUESTO DE REVISTA No. <?=$v->id?> <?=(count($bill)>0)?' FACTURA '.$bill->factura_id:''?></b></h1>
            <?php else: ?>
            <h1><b>ORDEN DE REVISTA No. <?=$orden->ord_id?></b></h1>
            <?php endif; ?>
            <table id="head" >
                <tr>
        ...
        <main>
            ...
            <table id="aviso" style="page-break-inside:avoid; margin-bottom: 0px;">
                <tbody>
                        <tr>
                            <td style="text-align:left;border-top: 1px solid #C1CED9;padding-top:5px;width:33.33%">TAMAÑO : <?=$row->drevis_tamano?></td>
                            <td style="text-align:left;border-top: 1px solid #C1CED9;padding-top: 5px;width:33.33%">FRECUENCIA : <?=$row->drevis_frecuencia?></td>
                            <td style="text-align:left;border-top: 1px solid #C1CED9;padding-top: 5px;width:33.33%">No. AVISOS : <?=$row->drevis_numavisos?></td>
                        </tr>
                        <tr>
                            <td style="text-align:left">TITULO : <?=$row->drevis_titulo?></td>
                            <td style="text-align:left">POSICIÓN : <?=$row->drevis_posicion?></td>
                            <td style="text-align:left">TARIFA  : <?=number_format($row->drevis_tarifa,0,"",".")?></td>
                        </tr>
                        <tr>
                            <td style="text-align:left">TINTA : <?=$row->tinta?></td>
                            <td style="text-align:left">FECHA INSERCIÓN : <?=$row->drevis_fechinser?></td>
                            <td style="text-align:left">VALOR TOTAL : <?=number_format($row->drevis_total,0,"",".")?></td>
                        </tr>
                        <?php if(!empty($row->drevis_detalle)): ?>
                        <tr>
                            <td style="text-align:left" colspan="3">DETALLE : <?=$row->drevis_detalle?></td>
                        </tr>
                        <?php endif; ?>
                    <?php 
