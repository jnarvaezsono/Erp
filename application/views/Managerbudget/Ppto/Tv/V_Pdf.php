...
<html lang="en">
    ...
    <body>
        <header class="clearfix">
            ...
            <?php if($printOrder == 0): ?>
            <h1>PRESUPUESTO DE TELEVISIÓN No. <?=$v->id?> <?=(count($bill)>0)?' FACTURA '.$bill->factura_id:''?></h1>
            <?php else: ?>
            <h1>ORDEN DE TELEVISIÓN No. <?=$orden->ord_id?></h1>
            <?php endif; ?>
            <table id="head" >
                <tr>
            ...
            <table id="aviso" >
                ...
                <tbody>
                    <?php foreach ($detail as $d) : ?>
                        <tr>
                            <td class="desc"><?= str_replace(',', ' ', $d->dtv_fechasalida) ?></td>
                            <td class="qty"><?=$d->dtv_programa?></td>
                            <td class="qty"><?=$d->dtv_hora?></td>
                            <td class="qty"><?=$d->dtv_franja?></td>
                            <td class="qty"><?=$d->dtv_referencia?></td>
                            <td class="qty"><?=$d->dtv_segundo?></td>
                            <td class="unit">$<?=number_format($d->dtv_total,0,"",".")?></td>
                        </tr>
                        <?php if(!empty($d->dtv_detalle)): ?>
                            <tr style="border-bottom: 1px solid #C1CED9;">
                                <td style="text-align:left; border-bottom: 1px solid #C1CED9;" colspan="7">DETALLE : <?=$d->dtv_detalle?></td>
                            </tr>
                            <?php else: ?>
                            <tr style="border-bottom: 1px solid #C1CED9;"><td colspan="7"></td></tr>
                        <?php endif; ?>
                    <?php 
                    ...
                    <tr>
                        <td colspan="6">Descuento <?=$v->descuento?>%</td>
                    ...
                    <tr>
                        <td colspan="6">SPA <?=$v->porcentaje_spa?>%</td>
                        <td class="total">$<?=number_format($vlrSpa,0,"",".")?></td>
                    </tr>
                    <tr>
                        <td colspan="6">IVA SPA <?=$v->porcentaje_iva_spa?>%</td>
