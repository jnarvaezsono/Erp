...
<div class="content-wrapper">
    <section class="content-header">
        <h1>
            <i class="fa fa-edit"></i> ORDEN DE COSTO N&deg;
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control input-sm decimals" id="ordcos_desc" name="ordcos_desc" value="<?= $row->descuento ?>" placeholder="0">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <div class="input-group">
                                        <span class="input-group-addon">%</span>
                                        <input type="text" class="form-control input-sm decimals" id="ordcos_iva" name="ordcos_iva" value="<?= $row->iva ?>"  placeholder="0">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <div class="input-group">
                                        <span class="input-group-addon">$.</span>
                                        <input readonly type="text" class="form-control input-sm decimals" id="ordcos_valor" value="<?= $row->valor ?>" placeholder="$ 0">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                    ...
                                    <div class="row">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control input-sm info-required" id="tpsv_id" name="tpsv_id">
                                                    ...
                                                    <?php foreach ($servicios as $v) : ?>
                                                        <option value="<?= $v->id_tipo_servicio ?>" <?= ($v->id_tipo_servicio == $row->servicio) ? 'selected' : '' ?>><?= $v->nombre ?> (<?= $v->tabla ?>)</option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <div class="input-group input-group-sm">
                                                    <input readonly id="ordcos_noorden" name="ordcos_noorden" type="text" class="form-control" value="<?=$row->presupuestos?>">
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <textarea type="text" class="form-control input-sm" rows="2" id="ordcos_observa" name="ordcos_observa" ><?= $row->observacion ?></textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<div class="modal fade" id="modal-ppto">
    <div class="modal-dialog" >
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Cerrar</button>
                <button type="button" class="btn btn-primary pull-right"  onclick="AddPpto()"><i class="fa fa-save"></i> Actualizar</button>
            </div>
        </div>
    </div>
</div>
<script>
    ...
    function OpenDetail(id_detalle){
        $.post('<?= base_url() ?>Managerbudget/C_Ppto/GetRowDetailppto',{table:'det_ordcostos',field:'dordcos_id',id_detalle:id_detalle},function(data){
            
            $('#dordcos_detalle').val(data.dordcos_detalle);
            $('#dordcos_cant').val(data.dordcos_cant);
            $('#dordcos_total').autoNumeric('set', data.dordcos_total);
            $('#dordcos_valor').autoNumeric('set', data.dordcos_valor);
            $('#btn-create').hide();
            $('#btn-update').show();
    ...
    function UpdateDetail(id_detalle){
        if (validatefield('det-required')) {
            ...
            var valor = ($('#dordcos_valor').val() == '') ? 0 : $('#dordcos_valor').val().replace(/\./g, '').replace(/,/g, '.');
            var total = $('#dordcos_total').val().replace(/\./g, '').replace(/,/g, '.');
            $.post('<?= base_url() ?>Managerbudget/O_Cost/C_Order_Cost/UpdateDetail',{
                dordcos_id:id_detalle,
                ordcos_id:<?= $id ?>,
                dordcos_valor:valor,
                dordcos_total:total,
                dordcos_detalle:$('#dordcos_detalle').val(),
    ...
    function DeleteDetail(id_detalle,total){
        swal({
            ...
            if (result) {
                $.post('<?= base_url() ?>Managerbudget/O_Cost/C_Order_Cost/DeleteDetail',{
                    total:total,
