...
<script>
    ...
    function CopyMasive(){
        ...
        if(pptos != ''){
            $.post('<?= base_url() ?>Managerbudget/C_Preorden/CopyMasive',{pptos:pptos,tipo:<?= $table ?>},function(data){
                if(data.errors != ''){
    ...
    function ConvertirPpto(preorden,tipo, tabla){
        swal({
            title: 'Confirma la creación del presupuesto a partir de la pre orden '+preorden+'?',
            text: "",
            type: 'warning',
    ...
    function EditPpto(id,tipo){
        switch(tipo){
            case 1:
                window.location.replace("<?= base_url() ?>Prensa/EditOrden/" + id + "/<?= $table ?>");
                break;
            case 2:
                window.location.replace("<?= base_url() ?>Clasificado/EditOrden/" + id + "/<?= $table ?>");
                break;
            case 3:
                window.location.replace("<?= base_url() ?>Revista/EditOrden/" + id + "/<?= $table ?>");
                break;
            case 4:
                window.location.replace("<?= base_url() ?>Radio/EditOrden/" + id + "/<?= $table ?>");
                break;
            case 5:
                window.location.replace("<?= base_url() ?>Tv/EditOrden/" + id + "/<?= $table ?>");
                break;
            case 6:
                window.location.replace("<?= base_url() ?>Externa/EditOrden/" + id + "/<?= $table ?>");
                break;
            case 7:
                window.location.replace("<?= base_url() ?>Interna/EditOrden/" + id + "/<?= $table ?>");
                break;
            case 8:
                window.location.replace("<?= base_url() ?>Exterior/EditOrden/" + id + "/<?= $table ?>");
                break;
            case 9:
                window.location.replace("<?= base_url() ?>Impreso/EditOrden/" + id + "/<?= $table ?>");
                break;
            case 10:
                window.location.replace("<?= base_url() ?>Articulo/EditOrden/" + id + "/<?= $table ?>");
                break;
        }
    }
    ...
    function NewPpto(){
        switch(<?= $table ?>){
            case 1:
                window.location.replace("<?= base_url() ?>Prensa/NewOrden/<?= $table ?>");
                break;
            case 2:
                window.location.replace("<?= base_url() ?>Clasificado/NewOrden/<?= $table ?>");
                break;
            case 3:
                window.location.replace("<?= base_url() ?>Revista/NewOrden/<?= $table ?>");
                break;
            case 4:
                window.location.replace("<?= base_url() ?>Radio/NewOrden/<?= $table ?>");
                break;
            case 5:
                window.location.replace("<?= base_url() ?>Tv/NewOrden/<?= $table ?>");
                break;
            case 6:
                window.location.replace("<?= base_url() ?>Externa/NewOrden/<?= $table ?>");
                break;
            case 7:
                window.location.replace("<?= base_url() ?>Interna/NewOrden/<?= $table ?>");
                break;
            case 8:
                window.location.replace("<?= base_url() ?>Exterior/NewOrden/<?= $table ?>");
                break;
            case 9:
                window.location.replace("<?= base_url() ?>Impreso/NewOrden/<?= $table ?>");
                break;
            case 10:
                window.location.replace("<?= base_url() ?>Articulo/NewOrden/<?= $table ?>");
                break;
        }
    ...
    function  addOrder(id){
        ...
        swal({
            ...
            preConfirm: (order) => {
            return fetch('<?= base_url() ?>Managerbudget/C_Preorden/AddOrder/'+id+'/<?= $table ?>/'+order)
              .then(response => {
                if (!response.ok) {
    ...
    function Anule(id){
        swal({
            ...
            if (result) {
                $.post('<?= base_url() ?>Managerbudget/C_Preorden/Anule',{ppto:id,tipo:<?= $table ?>,status:'9999'},function(data){
                    if(data.res == 'OK'){
    ...
    function copy(id, cliente, total){
        swal({
            ...
            if (result) {
                $.post('<?= base_url() ?>Managerbudget/C_Preorden/Copy',{ppto:id,tipo:<?= $table ?>,cliente:cliente,total:total,status:'9999'},function(data){
                    if(data.res == 'OK'){
                        alertify.success('OK');
                        Filtrar();
    ...
    function Cargar_Tabla() {
        ...
        var oTable = $('#table_ppto').dataTable({
            ...
            "buttons": [],
            "ajax": {
                "url": "<?= base_url() ?>Managerbudget/C_Preorden/GetListTable/" + type + '/' + id+ '/' + fecha_ini + '/' + fecha_fin + '/'+proveedor,
                "dataSrc": "datos"
            },
            "language": {
