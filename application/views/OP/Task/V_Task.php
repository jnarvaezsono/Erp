        ...
        <div class="box">
            <div class="box-header with-border">
                <div class="user-block">
                    ...
                    <span class="description" style="margin-left: 15px;"><?= $infoOP->camp_nombre ?> - <?= $infoOP->pdcl_nombre ?> - <div class="box-title title-box">Tarea N&deg;<?= $id ?> - <?= " OP N&deg; " . $info->id_op ?><?= (!empty($info->presupuesto)) ? " - Presupuesto N&deg; " . $info->presupuesto : "" ?></div><div class="pull-right" id="div-status"></div><div class="pull-right" id="div-back" onclick="history.back()"><i  class="fa fa-fw fa-backward"></i> </div></span>
                </div>
            </div>
            <div class="box-body">
                <div class="row">
                    <div class="col-md-12" style="text-align:center">
                        <?= (!empty($BtnChangeResponsable)) ? '<button type="button" style="display:none" class="btn btn-sm btn-opt btn-default  reasig-task" onclick="UpdateTask(false,'.$info->id_op.')"><i class="fa fa-refresh"></i> Reasignar </button>' : "" ?>
                        <?= (!empty($BtnUpdateTask)) ? '<button type="button" style="display:none" class="btn btn-sm btn-opt btn-default  update-task" onclick="UpdateTask(true,'.$info->id_op.')"><i class="fa fa-refresh"></i> Actualizar Tarea</button>' : "" ?>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <textarea class=" input-task "  id="descripcion" name="descripcion" placeholder="Enter ..."><?= $info->descripcion ?></textarea>
                        </div>
                    </div>
                            ...
                            <div class="row">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control input-sm input-task task-required select-modal "  id="modalidad_cobro" name="modalidad_cobro" >
                                            ...
                                            <?php foreach ($modalidades as $v) : ?>
                                                <option value="<?= $v->descripcion ?>" <?= ($info->modalidad_cobro == $v->descripcion) ? "selected" : "" ?>><?= $v->descripcion ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control input-sm select-modal input-task task-required " id="id_unidad" name="id_unidad" onchange="LoadSelect('t_responsable', this.value)">
                                            <option value="">. . .</option>
                                            <?php foreach ($unidades as $v) : ?>
                                                <option value="<?= $v->id_unidad ?>" area="<?= $v->area ?>" <?= ($info->id_unidad == $v->id_unidad) ? "selected" : "" ?>><?= $v->descripcion ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control   input-task select-modal task-required input-reasig" multiple id="t_responsable" name="id_responsable[]" >
                                            <option value="">. . .</option>
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <div class="input-group date">
                                            ...
                                            <input type="text" id="fecha_entrega" value="<?= $info->fecha_entrega ?>" name="fecha_entrega" class="form-control pull-right  input-task task-required picker" >
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-9">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control input-sm input-task select-modal input-reasig" multiple id="notificados" name="notificados[]" >
                                            <option value="">. . .</option>
                                            <?php foreach ($notificados as $v) : ?>
                                                <option value="<?= $v->id_users ?>"><?= $v->name ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                            ...
                            <div class="row">
                                <div class="col-md-3" style="display:none">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control input-sm input-task task-required select-modal input-reasig" id="area_responsable" name="area_responsable" >
                                            <option value="">. . .</option>
                                            <?php foreach ($area_responsable as $v) : ?>
                                                <option value="<?= $v->id_area ?>" <?= ($info->area_responsable == $v->id_area) ? "selected" : "" ?>><?= $v->descripcion ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control  select-modal input-task task-required " id="id_categoria" name="id_categoria" onchange="LoadForm(this.value)" >
                                            <option value="">. . .</option>
                                            <?php foreach ($categorias as $v) : ?>
                                                <option value="<?= $v->id_categoria ?>" form="<?= $v->form ?>" <?= ($info->id_categoria == $v->id_categoria) ? "selected" : "" ?>><?= $v->descripcion ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                </div>
...
$(function () {
    ...
    if ('<?= $info->notificados ?>' != '') {
        var notificados = '<?= $info->notificados ?>';
        var array_notificados = notificados.split(',');
        $("#notificados").val(array_notificados);
    }

    if ('<?= $info->id_responsable ?>' != '') {
        var responsable = '<?= $info->id_responsable ?>';
        var array_responsable = responsable.split(',');
        $("#t_responsable").val(array_responsable);
    }
    ...
    function Lock(estado) {
        ...
        if(estado == 1){
            if($('#modalidad_cobro').val() == 'COTIZAR'){
                $(".attachedCot").show();
            }
            $(".update-task, .attached, .reasig-task,  #btn-comment, #NewTaskPhoto").show();
            ...
            $(".update-task, .attached, #btn-comment, .reasig-task").show();
            ...
            $(".update-task, .attached, #btn-comment, .reasig-task, #NewTaskPhoto").show();
    ...
    function UpdateTask(opcion,op) {
            ...
            if (validatefield('task-required')) {
                ...
                formData.append("descripcion", CKEDITOR.instances['descripcion'].getData());//3
